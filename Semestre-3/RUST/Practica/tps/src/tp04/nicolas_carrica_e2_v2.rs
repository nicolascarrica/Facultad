...
// get_historial_compras(id: id_cliente, monto_minimo: f64) -> ??? // Cree un struct InfomreCompra con los campos correspondientes mas arriba
    ...
    fn get_historial_compras(&self, id_cliente: &str, monto_minimo: f64) -> Result<Vec<InformeCompra>, String> {
        let mut compras: Vec<InformeCompra> = self.ventas.iter()
            .filter(|venta| venta.cliente.info.dni == id_cliente)
            .filter_map(|venta| {
                let total = venta.calcular_precio();
        ...
        if compras.is_empty() {
            Err(format!("El cliente con DNI {} no posee compras mayores a ${}", id_cliente, monto_minimo))
        } else {
...
mod test {
    ...
    fn test_reporte_ventas() {
        ...
        if let Some((cat_map, vend_map)) = sistema.reporte_ventas() {
            ...
            panic!("Se esperaba un reporte de ventas, pero se obtuvo None");
        }
    }

    #[test]
    fn test_get_historial_compras_exitoso() {
        let mut sistema = Sistema::new();
        let cliente = crear_cliente(true);
        let dni = cliente.info.dni.clone();
        let vendedor = crear_vendedor();
        ...
        sistema.crear_venta(
            ...
            vec![(Producto::new("Heladera", Categoria::Electrodomesticos, 1000.0), 1)],
        );
        ...
        sistema.crear_venta(
            ...
            vendedor.clone(),
            MedioDePago::Efectivo,
            vec![(Producto::new("Cafetera", Categoria::Electrodomesticos, 800.0), 1)],
        );
        ...
        sistema.crear_venta(
            ...
            vec![(Producto::new("Mesa", Categoria::Hogar, 1000.0), 1)],
        );

        let resultado = sistema.get_historial_compras(&dni, 600.0);

        if let Ok(historial) = resultado {
            assert_eq!(historial.len(), 2);

            // Comprobar orden cronológico (2/1 < 3/1)
            assert_eq!(historial[0].fecha, Fecha::new(2, 1, 2025));
            assert_eq!(historial[1].fecha, Fecha::new(3, 1, 2025));

            assert_eq!(historial[0].monto_total, 720.0);
            assert_eq!(historial[1].monto_total, 630.0);
        } else {
            panic!("Se esperaba un Ok con historial, pero se recibió un Err");
        }
    }

    #[test]
    fn test_get_historial_compras_error_sin_resultados() {
        let mut sistema = Sistema::new();
        let cliente = crear_cliente(true);
        let dni = cliente.info.dni.clone();
        let vendedor = crear_vendedor();
    
        // Solo una venta, que no supera el mínimo
        sistema.crear_venta(
            1,
            Fecha::new(1, 1, 2025),
            cliente,
            vendedor,
            MedioDePago::Efectivo,
            vec![(Producto::new("Batidora", Categoria::Hogar, 500.0), 1)],
        ); // 400 * 0.9 = 360
        ...
        if let Err(mensaje) = resultado {
            assert_eq!(
                mensaje,
                format!("El cliente con DNI {} no posee compras mayores a ${}", dni, 600.0)
            );
        } else {
            panic!("Se esperaba un Err pero se recibió Ok");
        }
    }
