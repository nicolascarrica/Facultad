...
begin
  if (a = nil) then
    ...
    // El nodo actual se cuenta y se exploran ambos subárboles porque puede haber valores menores en ambos.
    CantidadDeCodigosMenores := 1 + CantidadDeCodigosMenores(a^.HI, cod) + CantidadDeCodigosMenores(a^.HD, cod)
  else
    // Si el nodo actual no es menor que `cod`, solo se explora a la izquierda.
...
begin
  if (a = nil) then
    ...
    // El nodo actual está entre los dos códigos, se suma su monto total y se sigue explorando ambos subárboles.
    ...
    // El nodo actual y todos en el subárbol izquierdo no cumplirán, se sigue explorando a la derecha.
    ObtenerMontoTotalEntreDosCodigos := ObtenerMontoTotalEntreDosCodigos(a^.HD, codigo1, codigo2)
  else
    // El nodo actual y todos en el subárbol derecho no cumplirán, se sigue explorando a la izquierda.
